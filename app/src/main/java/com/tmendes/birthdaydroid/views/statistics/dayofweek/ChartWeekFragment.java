...
public class ChartWeekFragment extends AbstractStatisticFragment implements OnChartValueSelectedListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        final String label = requireContext().getResources().getString(R.string.menu_statistics_week);
        title.setText(label);
        ...
        xAxis.setValueFormatter(new DayOfWeekLabelFormatter(getResources()));
    ...
    protected void updateContacts(List<Contact> contacts) {
        ...
        for (DayOfWeek dayOfWeek : DayOfWeek.values()) {
            Integer count = dayOfWeekStats.get(dayOfWeek);
            if (count == null) {
                count = 0;
            }
            BarEntry entry = new BarEntry(dayOfWeek.getValue(), count);
    ...
    protected int getCorrespondingTextOrDiagramNavId() {
        return R.id.nav_statistics_week_text;
    }
}